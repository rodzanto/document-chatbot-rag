...
def get_kendra_client(
    ...
    profile_name = 'default' #os.environ.get("AWS_PROFILE")
    if profile_name:
        print(f"  Using profile: {profile_name}")
        session_kwargs["profile_name"] = profile_name
    ...
    kendra_client = session.client(
        service_name="kendra",
        config=retry_config,
        **client_kwargs
    )

    print("boto3 Kendra client successfully created!")
    print(kendra_client._endpoint)
